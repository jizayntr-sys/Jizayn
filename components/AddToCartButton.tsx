...
export default function AddToCartButton({ url, platform = 'Shopier' }: AddToCartButtonProps) {
  ...
  return (
    <a
      ...
      className={`
        relative flex-1 min-w-[140px] px-4 py-2.5 rounded-full font-bold text-white transition-all duration-500 ease-out text-sm
    ...
    >
      {icon}
      <span className="relative z-10 font-semibold">{platform}</span>
      <ExternalLink className="w-3.5 h-3.5 opacity-70 group-hover:translate-x-1 group-hover:opacity-100 transition-all duration-300" />
    </a>
  );
}